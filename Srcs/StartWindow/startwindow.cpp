    ...
    , ui(new Ui::start_window_widget)
{
	...
	// 点击创建新的库文件
	connect(ui->create_new_kl_button, &QPushButton::clicked, this, &StartWindow::createNewKLButtonClicked);
	// 点击listWidget中的item，就进入到这个item对应的知识库
	connect(ui->listwidget, &QListWidget::itemClicked, this, &StartWindow::listWidgetItemClicked);
	
	// 搜索框被修改后，检查搜索框内的文本是否是空，如果是空则加载历史
	connect(ui->search_lineEdit, &QLineEdit::textChanged, this, &StartWindow::checkSearchLineEdit);

	// 点击搜索按钮
	connect(ui->search_pushButton, &QPushButton::clicked, this, &StartWindow::searchButtonClicked);
	// 搜索框回车
	connect(ui->search_lineEdit, &QLineEdit::returnPressed, this, &StartWindow::searchButtonClicked);

	// 点击排序按钮
	connect(ui->sort_button, &QPushButton::clicked, this, &StartWindow::sortButtonClicked);
}
...
// 加载两个listWidget
void StartWindow::refreshListWidget()
{
	ui->listwidget->clear();
	...
	if (!recent_kl_list.open(QIODevice::ReadOnly))
	{
		...
		if (!recent_kl_list.open(QIODevice::WriteOnly)) {
			QMessageBox::warning(this, "错误", "无法打开或操作文件：" + recent_kl_list_path);
			return;
		}
...
// 槽：点击创建新的库文件
void StartWindow::createNewKLButtonClicked()
{
	// 打开新的库文件创建窗口
...
void StartWindow::listWidgetItemClicked(QListWidgetItem* item)
{
	// 获取点击的知识库名称和路径
	...
	else
	{
		...
		if (reply == QMessageBox::Yes) {
			...
			// 刷新listWidget
			StartWindow::refreshListWidget();
		}
	}
}

// 槽：点击搜索按钮
void StartWindow::searchButtonClicked() {
	// 获取搜索文本  
...
void StartWindow::checkSearchLineEdit()
{
	// 这里是历史库就不需要刷新了
	if (ui->search_lineEdit->text().isEmpty() && ui->hint_label->text() != "最近打开的知识库：")
	{
		ui->listwidget->clear();
		ui->hint_label->setText("最近打开的知识库：");
		StartWindow::refreshListWidget();  // 刷新两个listWidget
	}
}

// 槽：点击排序按钮，对listWidget中的item进行字典序排序
void StartWindow::sortButtonClicked()
{
